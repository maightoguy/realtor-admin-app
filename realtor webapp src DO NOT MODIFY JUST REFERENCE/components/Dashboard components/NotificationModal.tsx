...
const getNotificationIcon = (type: string) => {
  ...
  if (normalizedType.includes("approv") || normalizedType.includes("success")) {
    return (
      <svg
        width="26"
        height="26"
  ...
  // Default to new_account / info icon for auth, account, or any other type
  return (
    <svg
      width="32"
      height="32"
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 bg-white z-50 md:inset-auto md:top-20 md:right-8 md:w-[440px] md:max-h-[80vh] md:rounded-2xl md:shadow-2xl flex flex-col">
        ...
        {/* Header for Mobile */}
        <div className="relative flex md:hidden items-center p-4 border-b border-gray-200">
          <button
            onClick={onClose}
            className="w-14 h-14 flex items-center justify-center rounded-full hover:bg-gray-100 active:bg-gray-200 -ml-2 z-10"
          >
            {/* Back arrow*/}
            <ArrowLeft className="w-5 h-5 text-gray-600" />
          </button>
          <h2 className="absolute inset-0 flex items-center justify-center text-lg font-semibold text-gray-800 pointer-events-none">
            Notifications
          </h2>
        </div>
          ...
          {loading ? (
            ...
            notifications.map((item, index) => (
              ...
              >
                ...
                <div
                  className={`flex-shrink-0 mt-1 ${item.seen ? "ml-4" : ""}`}
                >
                ...
                <div className="flex-grow">
                  ...
                  {!!item.metadata?.attachment && (
                    // Assuming attachment is stored in metadata like { attachment: { name: '...', size: '...' } }
                    // If the structure differs, we'll need to adjust.
                    <div className="mt-2 flex items-center gap-2 p-2 bg-gray-50 rounded-md border border-gray-200 w-fit">
                      <Clock className="w-4 h-4 text-gray-400" />
                      <span className="text-xs font-medium text-gray-700">
                        {(item.metadata?.attachment as any)?.name ||
                          "Attachment"}
                      </span>
                      <span className="text-xs text-gray-400">
                        {(item.metadata?.attachment as any)?.size || ""}
                      </span>
                    </div>
                  )}
