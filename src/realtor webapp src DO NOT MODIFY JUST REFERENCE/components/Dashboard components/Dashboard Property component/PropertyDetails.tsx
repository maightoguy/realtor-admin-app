...
const PropertyDetails: FC<PropertyDetailsProps> = ({
  ...
  const handleSubmitReceipt = async (data: {
    ...
    try {
      ...
      // 1. Upload files
      const uploadedUrls = await receiptService.uploadReceiptFiles(data.files);
      ...
      await receiptService.createReceipt({
        ...
        amount_paid: Number(data.amount.replace(/[^0-9.]/g, "")),
  ...
  const truncatedDescription = (() => {
    const slice = descriptionText.slice(0, descriptionMaxLength);
    const lastSpaceIndex = slice.lastIndexOf(" ");
    const safeSlice =
      lastSpaceIndex > 120 ? slice.slice(0, lastSpaceIndex) : slice;
